...
fn new_empty_test() {
    ...
    for row in 0..rows {
        for column in 0..columns {
            assert_eq!(
                true,
                FloatingPoint::equals(matrix.elements[row][column], 0.0)
            );
        }
    }
}
...
#[test]
fn determinant_test() {
    let elements = vec![vec![1.0, 5.0], vec![-3.0, 2.0]];
...
fn submatrix_test() {
    ...
    let expected = Matrix::new(2, 2, Option::Some(vec![vec![-3.0, 2.0], vec![0.0, 6.0]]));
